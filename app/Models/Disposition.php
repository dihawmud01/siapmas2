...
class Disposition extends Model
{
    ...
    protected $fillable = ['to', 'due_date', 'content', 'note', 'letter_status', 'letter_id', 'user_id'];
    ...
    public function scopeSearch($query, $search)
    {
        return $query->when($search, function ($query, $find) {
            return $query->orWhere('content', 'LIKE', '%' . $find . '%')->orWhere('to', 'LIKE', $find . '%');
        });
    }
    ...
    public function scopeRender($query, Letter $letter, $search)
    {
        ...
        return $query
            ...
            ->when($letter, function ($query, $letter) {
                return $query->where('letter_id', $letter->id);
            })
            ->latest('created_at')
